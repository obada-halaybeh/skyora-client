...
const TABS = ['Upcoming', 'Past Trips', 'Cancelled']
        ...
        <div className="space-y-4">
          {trips.map((trip) => (
            <div key={trip.id}>
              <TripCard
                trip={trip}
                tab={tabKey === 'past' ? 'past' : tabKey === 'cancelled' ? 'cancelled' : 'upcoming'}
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {SETTINGS_CARDS.map((card) => (
              <button
                key={card.title}
                className="bg-white rounded-xl border border-hairline p-5 text-left hover:shadow-card-hover transition-shadow"
              >
